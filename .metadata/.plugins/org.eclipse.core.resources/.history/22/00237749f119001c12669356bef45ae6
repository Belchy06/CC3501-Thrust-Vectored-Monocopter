...
void get_position(GPS *self, float *lat, float *lon, unsigned long *age) {
	*lat = self->_latitude == GPS_INVALID_ANGLE ? GPS_INVALID_F_ANGLE : (self->_latitude / 10000.0f);
	*lon = self->_longitude == GPS_INVALID_ANGLE ? GPS_INVALID_F_ANGLE : (self->_longitude / 10000.0f);
}

void get_altitude(GPS *self, float *alt) {
	*alt = self->_altitude == GPS_INVALID_ALTITUDE ? GPS_INVALID_F_ALTITUDE : self->_altitude / 100.0f;
}
...
char *cardinal(GPS *self, float course) {
