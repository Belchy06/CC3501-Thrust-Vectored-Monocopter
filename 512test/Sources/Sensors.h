...
typedef struct BNO085 {
	...
	uint16_t memsRawMagX, memsRawMagY, memsRawMagZ;bool _hasReset;
	...
	void (*enableLinearAccelerometer)(struct BNO085 *self,
			uint16_t timeBetweenReports);
	void (*enableGyro)(struct BNO085 *self, uint16_t timeBetweenReports);
	void (*enableMagnetometer)(struct BNO085 *self,
			uint16_t timeBetweenReports);
	void (*enableGameRotationVector)(struct BNO085 *self,
			uint16_t timeBetweenReports);bool (*dataAvailable)(
			struct BNO085 *self);
	...
	void (*getAccel)(struct BNO085 *self, float *x, float *y, float *z,
			uint8_t *accuracy);
	void (*getLinAccel)(struct BNO085 *self, float *x, float *y, float *z,
			uint8_t *accuracy);
	void (*getGyro)(struct BNO085 *self, float *x, float *y, float *z,
			uint8_t *accuracy);
	void (*getMag)(struct BNO085 *self, float *x, float *y, float *z,
			uint8_t *accuracy);
...
typedef struct GPS {
	...
	uint8_t _parity;bool _is_checksum_term;
	uint8_t _term[15];
	uint8_t _sentence_type;
	uint8_t _term_number;
	uint8_t _term_offset;bool _gps_data_good;

	bool (*encode)(struct GPS *self, char c);bool (*term_complete)(
			struct GPS *self);
	void (*get_position)(struct GPS *self, float *lat, float *lon,
			unsigned long *age);
	void (*get_altitude)(struct GPS *self, float *alt);
	float (*distance_between)(struct GPS *self, float lat1, float long1,
			float lat2, float long2);
	float (*course_to)(struct GPS *self, float lat1, float long1, float lat2,
			float long2);
